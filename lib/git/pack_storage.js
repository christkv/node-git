...
PackStorage.prototype.unpack_deltified = function(packfile, type, offset, obj_offset, size, options) {
  ...
  // sys.puts("================================= base_offset: " + base_offset)
  // sys.puts("================================= offset: " + offset)
  // sys.puts("================================================ unpack object")
  ...
  // sys.puts("================================================ unpacked object")
  // If it's not a Commit or Tree return an empty delta
  if(!(type == OBJ_COMMIT || type == OBJ_TREE) && options['caching']) return [false, false];
  // sys.puts("================================================ start uncompress object")
  // Unpack the the data
  var delta = unpack_compressed(this, offset, size);
  // sys.puts(to_hex_string(delta));
  var delta2 = patch_delta(base, delta);
  // sys.puts("------------------------------------------ delta 2")
  // sys.puts(to_hex_string(delta2));  
  // sys.puts("================================================ finished uncompress object")
  return [delta2, type];
}
...
var patch_delta = function(base, delta) {
  ...
  // sys.puts("============================== delta.pos: " + pos)
  // sys.puts("============================== delta.length: " + delta.length)
  ...
  while(pos < delta.length) {
    ...
    if((c & 0x80) != 0) {
      ...
      pos = pos + 1;
      // sys.puts("====== (c & 0x80) != 0")
      // sys.puts("===================== cp_off: " + cp_off)
      // sys.puts("===================== cp_size: " + cp_size)
      // sys.puts(to_hex_string(base.substr(cp_off, cp_size)));
      ...
      // sys.puts("===================== pos: " + pos)
      // sys.puts("===================== c: " + c)      
      // sys.puts(to_hex_string(delta.substr(pos, c)));
  ...
  // sys.puts("=========================================== var patch_delta = function(base, delta)");
  return dest;
}
...
var init_pack = function(pack) {  
  ...
  with_idx(pack, function(err, idx) {
    ...
    for(var i = 0; i < FAN_OUT_COUNT; i++) {
      ...
      // sys.debug("=======================================================")      
      // 
      // sys.debug("hex: " + BinaryParser.hex(reverse_buffer(pos).toString('binary', 0, 4)));
      
      pos = BinaryParser.toInt(reverse_buffer(pos).toString('binary', 0, 4));
      // sys.puts(sys.inspect(pack.offsets))
      // sys.puts("--- pos: " + pos)
