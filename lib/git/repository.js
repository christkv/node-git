...
var get_raw_object_by_sha1 = function(repo, sha1o) {
  ...
  // sys.debug("=========================== sha: " + sha1o)
  // if(sha1o == "01a1b4f1cbf0f4d9c13618a3ca9c9f429c8dd0e8") {
  //   sys.debug(sys.inspect(repo.packs[0].offsets))
  // }
  ...
  // initpacks(repo);
  // sys.debug("============================================ __get_raw_object_by_sha1: ");  
  // BinaryParser.hprint(sha1)
  ...
  for(var i = 0; i < looses.length; i++) {
    ...
    // sys.debug("========================================= looses: " + ((o != null) ? o.type : ''));
    if(o) return o;
  }
  ...
  var packs = repo.packs;
  // sys.debug("==================================================================== PACKS")
  // sys.debug(sys.inspect(packs))
  
  for(var i = 0; i < packs.length; i++) {
    var o = packs[i].find(sha1);
    // sys.debug("========================================= packs again: " + ((o != null) ? o.type : ''));
    if(o != null) return o;
  }
...
Repository.prototype.ls_tree_path = function(sha, path, append) {
  // sys.puts("=========================================== Repository.prototype.ls_tree_path")
...
var walk_log = function(repo, sha, options, total_size) {
  // sys.debug("\n\n=== walk_log: " + sha + " - " + repo.already_searched[sha])  
  ...
  if(sha) {
    ...
    if(options["max_count"] == null || ((array.length + total_size) < options["max_count"])) {      
      // sys.puts("========================= max_count: " + options["max_count"])
      // sys.puts("========================= (array.length + total_size): " + (array.length + total_size))
      ...
      // sys.debug("============================= " + JSON.stringify(c.parent))
