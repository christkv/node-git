...
Repository.prototype.quick_diff = function(tree1, tree2, path, recurse) {
  ...
  if(t1) {
    Object.keys(t1['tree']).forEach(function(dir) {
      ...
      if(!t2_tree) {
        if(recurse) {
          changed.concat(self.quick_diff(hsh['sha'], null, full, true));
        } else {
          changed.push([full, 'added', hsh['sha', null]]);
        }
      } else if(hsh['sha'] != t2_tree['sha']) {
        if(recurse) {
          changed.concat(self.quick_diff(hsh['sha'], t2_tree['sha'], full, true));
        } else {
  ...
  if(t2) {
    Object.keys(t2['tree']).forEach(function(dir) {
      ...
      if(!t1_tree) {
        if(recurse) {
          changed.concat(self.quick_diff(null, hsh['sha'], full, true));
        } else {
