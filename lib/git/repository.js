...
var get_raw_object_by_sha1 = function(repo, sha1o) {
  ...
  // sys.debug("============================================ __get_raw_object_by_sha1: " + sha1o);  
  
  // Try packs
  var packs = repo.packs;
  // sys.debug("============================================ packs.length: " + packs.length)
  for(var i = 0; i < packs.length; i++) {
    var o = packs[i].find(sha1);    
    // sys.debug("========================================= packs: " + sys.inspect(o.type))
    if(o != null) return o;
  }

  // sys.debug("=========================================== init loose")
  if(!repo.loose) initloose(repo);
  // Try loose storage
  var looses = repo.loose;  
  // sys.debug("============================================ looses.length: " + looses.length)
  ...
  for(var i = 0; i < packs.length; i++) {
    var o = packs[i].find(sha1);
    // sys.debug("========================================= packs again: " + o)
    if(o != null) return o;
  }
  
  // sys.debug("=========================================== no sha found");
...
Repository.prototype.get_object_by_sha1 = function(sha1) {
  // sys.debug("============================================= before raw_for: " + sha1)
  var r = get_raw_object_by_sha1(this, sha1);
  // sys.debug("============================================= raw_for: " + sha1)
  if(!r) return null;
...
Repository.prototype.quick_diff = function(tree1, tree2, path, recurse) {
  ...
  // sys.puts("\n\nT1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1T1")
  // sys.puts("=============================== t1: " + sys.inspect(t1))  
  var t2 = tree2 ? this.list_tree(tree2) : null;
  // sys.puts("T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2T2")
  // sys.puts("=============================== t2: " + sys.inspect(t2))
...
Repository.prototype.list_tree = function(sha) {
  ...
  // sys.debug("============================= get_object_by_sha1: " + sha)
  var object = this.get_object_by_sha1(sha);
  // sys.debug("============================= get_object_by_sha1: ______________" + sha)
...
var walk_log = function(repo, sha, options, total_size) {
  // sys.debug("\n\n====================== var walk_log = function(repo, sha, options, total_size)")  
  // sys.debug("___________________________________________________________________ SEARCHED")
  // sys.debug(sys.inspect(repo.already_searched))
  ...
  if(sha) {
    // sys.debug("=============================== sha: " + sha)
    // sys.debug("=============================== total_size: " + total_size + "\n\n")
    // sys.debug("=============================== sha: " + sys.inspect(options["max_count"]))
    
    // Get the raw object
    o = get_raw_object_by_sha1(repo, sha);
    
    // sys.debug("=========================== type: " + o.type)
    ...
    if(options["max_count"] == null || ((array.length + total_size) < options["max_count"])) {
      ...
      for(var i = 0; i < c.parent.length; i++) {
        var psha = c.parent[i];
        // sys.puts("=================================================1")
        if(psha && !repo.files_changed(c.tree, repo.get_object_by_sha1(psha).tree, options["path_limiter"])) {
          // sys.debug("================================== psha: " + psha)
          add_sha = false;
        }        
        // sys.puts("=================================================2")
        ...
        // sys.debug("=============================== array.length: " + array.length);        
        // sys.debug("=============================== total_size: " + total_size);        
        // var r = walk_log(repo, psha, options, (array.length + total_size));
        // sys.puts("----------------------------------------------------------------- walk_log results")
        // sys.puts(sys.inspect(r))
        // sys.debug("===================================================== RRRRRRRRRRRRRRRRRR: " + r)
        subarray = subarray.concat(walk_log(repo, psha, options, (array.length + total_size))); 
        // sys.puts("======================================= subarray.length 111111 " + subarray.length)                 
        if(options["first_parent"]) break;
      }
      
      // sys.puts("====================== add_sha " +add_sha)
      
      if(options["path_limiter"] != null && add_sha) {
        output = c.raw_log(sha);
        // sys.puts("================================== push options[\"path_limiter\"] != null && add_sha")
        array.push([sha, output, c.comitter.date]);        
      }
      
      if(add_sha) {
        // sys.puts("======================================= subarray.length " + subarray.length)
...
Repository.prototype.rev_list = function(sha, options, callback) {
  try {
    ...
    // Walk the log
    // sys.debug("=============== sha: " + sha)
    // sys.debug("=============== options: " + sys.inspect(options))
    
    var log = this.log(sha, options);

    // sys.debug("-------------------------------------------------------------- log")
    // sys.debug(sys.inspect(log))
