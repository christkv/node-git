...
var get_raw_object_by_sha = function(repo, sha1o) {
  ...
  for(var i = 0; i < sha1o.length; i = i + 2) {
    ...
    // sha1 = sha1 + String.fromCharCode(parseInt(sha1o.substring(i, i + 2), 16));  
    var num = parseInt(sha1o.substr(i, 2), 16);
    sha1 = sha1 + String.fromCharCode(num);
  }
  ...
  // Try packs
  repo.packs.forEach(function(pack) {
    sys.puts("----------------------------- hello: " + sys.inspect(pack))


    var o = pack.find(sha1);
...
Repository.prototype.rev_list = function(sha, options, callback) {
  sys.puts("====== Repository.prototype.rev_list: " + sha)
  var end_sha = null;
