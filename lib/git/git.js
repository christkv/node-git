...
Git.prototype.blame_tree = function(commit, path, callback) {
  ...
  } else if(!path || path.constructor != String) {
    path = '';
  }
...
Git.prototype.looking_for = function(commit, path, callback) {
  ...
  self.rev_parse({}, commit, function(err, rev_parse_output) {
    ...
    self.repository.get_subtree(rev_parse_output, path, function(err, tree_sha) {
      ...
      sys.puts("====================================================== tree_sha: " + tree_sha)
      // Contains the files 
      var looking_for = [];
      // Fetch the object by the tree sha
      var entry = self.repository.get_object_by_sha1(tree_sha);
      sys.puts("========================================================= " + entry)
      return callback(err, entry);
      // self.repository.get_object_by_sha1(tree_sha, function(err, entry) {
