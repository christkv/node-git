...
Git.prototype.refs = function(options, prefix, callback) {
  ...
  // sys.puts("-- prefix: " + sys.inspect(prefix))
...
Git.prototype.ls_tree = function(treeish, paths, options, callback) {
  ...
  this.rev_parse({}, treeish, function(err, sha) {
    // Ls_tree
    callback(null, self.repository.ls_tree(sha, flatten(paths), options['r']));
  })
}
...
Git.prototype.diff = function(commit1, commit2, options, callback) {
  // try {
    ...
    // sys.puts("============================= commit1: " + commit1)
    // sys.puts("============================= commit2: " + commit2)
    // sys.puts("============================= options: " + options)
    ...
    qdiff.forEach(function(diff_arr) {
      ...
      // sys.debug("========================================================================== DATA OLD " + data_old.length)
      // data_old.forEach(function(l) { sys.debug(l)});
      // sys.debug("========================================================================== DATA NEW " + data_new.length)
      // data_new.forEach(function(l) { sys.debug(l)});
      // sys.puts("========================================================================== DATA OLD " + data_old.length)
      // data_old.forEach(function(l) { sys.puts(l)});
      // sys.puts("========================================================================== DATA NEW " + data_new.length)
      // data_new.forEach(function(l) { sys.puts(l)});

      // Get diffs
      var diffs = Difference.LCS.diff(data_old, data_new);
      if(diffs.length > 0) {        
        // sys.puts("========================================================================== DIFFS");
        // var _diffs = flatten(diffs);
        // _diffs.forEach(function(d) { sys.puts(d.to_a()[0] + ":" + d.to_a()[1] + ":" + d.to_a()[2]); });
        ...
        // sys.puts("====================== a_path: "+ a_path)
        // sys.puts("====================== b_path: "+ b_path)
        ...
        diffs.forEach(function(piece) {
          // sys.debug("======================================================= PIECE")
          // sys.puts(piece.to_a()[0] + ":" + piece.to_a()[1] + ":" + piece.to_a()[2]);
          // sys.debug(sys.inspect(piece))
          
          // try {
            // sys.puts("================================ " + Difference.LCS.Hunk)
        ...
        // sys.puts("========================= inspect: " + diffs.length)
        // sys.puts("========================= inspect: " + old_hunk)
        
        // Prepare next 
        // sys.debug("==========================================================================")
        // sys.debug(sys.inspect(old_hunk))
        output = output + old_hunk.diff(format);
        output = output + '\n';
...
//   }
// 
//   sys.puts(sys.inspect(Git.prototype))
//   // callback(null, null);
//   pre_loading_done = true
//   // sys.puts(sys.inspect(Git.prototype.status.toString()))
//   // var g = new Git("..../")
// 
//   // sys.debug("=====================================================================")
//   // sys.debug(index)
//   // sys.debug(sys.inspect(lines))
// });  
